 ...
 * @Date 2015/2/26
 */
@Toolbar(title = R.string.my_favorite)
public class MyFavoriteFragment extends ProgressFragment {
	@ID(id = R.id.list)
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		Cursor cursor = App.getResolver().query(DbTable.FILE_URI, DbTable.FILE_SELECTION, "collect=?", new String[] { String.valueOf(1) }, "ut DESC");
		if (null != cursor && 0 < cursor.getCount()) {
			mList.setAdapter(new MusicAdapter(getActivity(), cursor));
		} else {
			setEmptyIcon(R.drawable.no_favorite);
			setEmptyInfo(R.string.no_favorite);
			setEmptyShown(true);
		}
			...
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				...
				if (adapter instanceof CursorAdapter) {
					...
					if (null != cursor) {
						...
						intent.putExtra(
								PlayActivity.PLAY_MUSIC,
                                new Music(cursor.getInt(1),
                                        cursor.getLong(2),
                                        cursor.getString(3),
                                        cursor.getString(4),
                                        cursor.getString(5),
