package com.weishang.repeater.widget.noborder;
...
public class KenBurnsView extends FrameLayout {
    ...
    private void start(View view, long duration, float fromScale, float toScale, float fromTranslationX, float fromTranslationY, float toTranslationX, float toTranslationY) {
        ...
        com.nineoldandroids.view.ViewPropertyAnimator.animate(view).translationX(toTranslationX).translationY(toTranslationY).scaleX(toScale).scaleY(toScale).setDuration(duration);
    }
    ...
    public int getActiveImageResource() {
        int resId = -1;
        if (-1 != mActiveImageIndex && mActiveImageIndex < mResourceIds.length) {
            resId = mResourceIds[mActiveImageIndex];
        }
        return resId;
    }
